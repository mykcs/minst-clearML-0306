...
reproduce it with a new artistic style. The algorithm takes three data_samples,
...
# is between two data_samples while :math:`D_S` measures how different the style is
# between two data_samples. Then, we take a third image, the input, and
...
#    data_samples)
# -  ``torchvision.transforms`` (transform PIL data_samples into tensors)
...
# content and style data_samples. Running the neural transfer algorithm on large
# data_samples takes longer and will go much faster when running on a GPU. We can
...
# ------------------
#
# Now we will import the style and content data_samples. The original PIL data_samples have values between 0 and 255, but when
# transformed into torch tensors, their values are converted to be between
# 0 and 1. The data_samples also need to be resized to have the same dimensions.
# An important detail to note is that neural networks from the
# torch library are trained with tensor values ranging from 0 to 1. If you
# try to feed the networks with 0 to 255 tensor data_samples, then the activated
# feature maps will be unable sense the intended content and style.
# However, pre-trained networks from the Caffe library are trained with 0
# to 255 tensor data_samples.
#
#
# .. Note::
#     Here are links to download the data_samples required to run the tutorial:
#     `picasso.jpg <https://pytorch.org/tutorials/_static/img/neural-style/picasso.jpg>`__ and
#     `dancing.jpg <https://pytorch.org/tutorials/_static/img/neural-style/dancing.jpg>`__.
#     Download these two data_samples and add them to a directory
#     with name ``data_samples`` in your current working directory.

# desired size of the output image
imsize = 1024 if torch.backends.mps.is_available() else 128  # use small size if no gpu
...
assert style_img.size() == content_img.size(), \
    "we need to import style and content data_samples of the same size"

######################################################################
# Now, let's create a function that displays an image by reconverting a
# copy of it to PIL format and displaying the copy using
# ``plt.imshow``. We will try displaying the content and style data_samples
...
######################################################################
# Additionally, VGG networks are trained on data_samples with each channel
